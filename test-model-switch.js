...
async function main() {
  ...
  console.log(`✅ Got ${models.length} models across ${providers.length} providers`)
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      if (result.ok && result.data.ok !== false) {
        ...
        console.log(`      Resolved: ${resolved.modelProvider || '?'}/${resolved.model || '?'}`)
        results.push({ provider, model: testModel.name, status: 'success' })
      } else {
        console.log(`   ❌ Failed: ${result.data.error || 'Unknown error'}`)
        results.push({ provider, model: testModel.name, status: 'error', error: result.data.error })
      }
    } catch (err) {
      console.log(`   ❌ Exception: ${err.message}`)
      results.push({ provider, model: testModel.name, status: 'exception', error: err.message })
    }
    console.log('')
  }
