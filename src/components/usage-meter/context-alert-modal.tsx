...
function ContextAlertModalComponent({
  ...
  const barColor = isCritical ? 'bg-red-500' : isDanger ? 'bg-amber-500' : 'bg-amber-400'
  const iconBg = isCritical ? 'bg-red-100' : isDanger ? 'bg-amber-100' : 'bg-amber-100'
  const iconColor = isCritical ? 'text-red-600' : isDanger ? 'text-amber-600' : 'text-amber-600'
        ...
        <div className="px-6 pt-5 pb-6">
          ...
          <div className="flex items-start gap-3 mb-4">
            <div className={cn('rounded-full p-2 shrink-0', iconBg)}>
              <svg viewBox="0 0 24 24" className={cn('size-5', iconColor)} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div>
              ...
              <p className="text-xs text-primary-500 mt-0.5">
                {Math.round(contextPercent)}% of your model's context window is in use
              </p>
            </div>
          </div>
          ...
          <div className="bg-primary-50 rounded-lg p-3 mb-4">
            <p className="text-xs font-medium text-primary-800 mb-2">What does this mean?</p>
          ...
          <div className="space-y-2 mb-5">
            <p className="text-xs font-medium text-primary-800">Recommendations</p>
...
function Recommendation({ icon, text, emphasis }: { icon: string; text: string; emphasis?: boolean }) {
  return (
    <div className="flex items-start gap-2">
      <span className="text-xs shrink-0 mt-px">{icon}</span>
      <span className={cn('text-xs text-primary-600 leading-relaxed', emphasis && 'font-medium text-primary-800')}>
        {text}
      </span>
    </div>
  )
