...
export function ModeSelector({
  ...
  const handleApplyMode = useCallback((mode: Mode) => {
    // Check if mode requires model switch
    const needsModelSwitch = mode.preferredModel && mode.preferredModel !== currentModel;
    const modelAvailable = mode.preferredModel && availableModels.includes(mode.preferredModel);
  ...
  const handleConfirmApply = useCallback((switchModel: boolean) => {
    ...
    <>
      <div className="relative flex items-center gap-2" ref={selectorRef}>
        ...
        >
          ...
          {showDrift && <span className="text-yellow-600" title="Settings changed">⚠️</span>}
          {modelUnavailable && <span className="text-red-600" title="Model unavailable">⚠️</span>}
              ...
              {modes.length === 0 ? (
                ...
                <>
                  {modes.map((mode) => {
                    ...
                    const unavailable = mode.preferredModel && 
                    ...
                    return (
                      ...
                      >
                        ...
                        {drift && <span className="text-yellow-600 text-xs" title="Settings changed">⚠️</span>}
                        {unavailable && <span className="text-red-600 text-xs" title="Model unavailable">⚠️</span>}
                        {isApplied && !drift && (
